/** @odoo-module alias=@mail/discuss/call/common/rtc_service default=false */
...
import { callActionsRegistry } from "@mail/discuss/call/common/call_actions";
...
export class Rtc extends Record {
    ...
    async _initConnection() {
        intranetRtcLog("info", "Initializing RTC connection", {
    ...
    async handleRemoteTrack({ session, track, type, active = true }) {
        intranetRtcLog("info", "Handling remote media track", {
    ...
    async joinCall(channel, { audio = true, camera = false } = {}) {
        intranetRtcLog("info", "joinCall requested", {
        ...
        try {
            if (type === "camera") {
                if (this.state.sourceCameraStream) {
                    ...
                    intranetRtcLog("info", "Requesting camera media", summarizeBrowserRtcEnvironment());
                    sourceStream = await browser.navigator.mediaDevices.getUserMedia({
                        video: CAMERA_CONFIG,
                    });
                    intranetRtcLog("info", "Camera media acquired", {
                        tracks: sourceStream.getVideoTracks().map((track) => ({
                            id: track.id,
                            label: track.label,
                            enabled: track.enabled,
                            muted: track.muted,
            ...
            if (type === "screen") {
                if (this.state.sourceScreenStream) {
                    ...
                    intranetRtcLog("info", "Requesting screen media", summarizeBrowserRtcEnvironment());
                    sourceStream = await browser.navigator.mediaDevices.getDisplayMedia({
                        video: SCREEN_CONFIG,
                    });
                    intranetRtcLog("info", "Screen media acquired", {
                        tracks: sourceStream.getVideoTracks().map((track) => ({
                            id: track.id,
                            label: track.label,
                            enabled: track.enabled,
            ...
            intranetRtcLog("error", "Video media request failed", {
                type,
                name: error?.name,
                message: error?.message,
                browser: summarizeBrowserRtcEnvironment(),
    ...
    async resetAudioTrack({ force = false }) {
        ...
        if (force) {
            let audioTrack;
            try {
                intranetRtcLog("info", "Requesting microphone media", summarizeBrowserRtcEnvironment());
                const audioStream = await browser.navigator.mediaDevices.getUserMedia({
                    audio: this.store.settings.audioConstraints,
                });
                audioTrack = audioStream.getAudioTracks()[0];
                intranetRtcLog("info", "Microphone media acquired", {
                    id: audioTrack?.id,
                ...
                intranetRtcLog("error", "Microphone media request failed", {
                    name: error?.name,
                    message: error?.message,
                    browser: summarizeBrowserRtcEnvironment(),
                });
