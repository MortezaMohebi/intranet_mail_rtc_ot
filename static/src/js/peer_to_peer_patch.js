/** @odoo-module alias=@mail/discuss/call/common/peer_to_peer default=false */
...
function isIntranetRtcDebugEnabled() {
    try {
        return (
            window.localStorage?.getItem("intranet_mail_rtc_ot.debug") === "1" ||
            new URLSearchParams(window.location.search).get("intranet_rtc_debug") === "1"
        );
    } catch {
        return false;
    }
...
function intranetRtcLog(level, message, payload = undefined) {
    if (!isIntranetRtcDebugEnabled()) {
        return;
    }
    const logLevel = ["debug", "info", "warn", "error"].includes(level) ? level : "info";
    const prefix = "[intranet_mail_rtc_ot][rtc]";
    if (payload === undefined) {
        console[logLevel](prefix, message);
    } else {
        console[logLevel](prefix, message, payload);
    }
}
...
function summarizeIceServersForLog(iceServers) {
    ...
    return {
        ...
        servers: iceServers.map((server) => {
            ...
            return {
                urls: urlList.map((iceUrl) => String(iceUrl || "").replace(/\/\/.*@/, "//***@")),
...
function summarizeCandidateForLog(candidate) {
    if (!candidate) {
...
function summarizeBrowserRtcEnvironment() {
    return {
        href: window.location.href,
        protocol: window.location.protocol,
        host: window.location.host,
        isSecureContext: window.isSecureContext,
        hasMediaDevices: Boolean(browser.navigator.mediaDevices),
        hasGetUserMedia: Boolean(browser.navigator.mediaDevices?.getUserMedia),
        hasGetDisplayMedia: Boolean(browser.navigator.mediaDevices?.getDisplayMedia),
        hasRTCPeerConnection: Boolean(window.RTCPeerConnection),
        hasMediaStream: Boolean(window.MediaStream),
    };
}

function sanitizeIntranetIceServers(iceServers) {
    if (!Array.isArray(iceServers)) {
        return [];
    }
    return iceServers
        .map((server) => {
            const urls = server?.urls ?? server?.url ?? [];
            const urlList = Array.isArray(urls) ? urls : [urls];
            const cleanUrls = urlList.filter((iceUrl) => {
                const value = String(iceUrl || "").toLowerCase();
                return value && !value.includes("google") && !value.includes("twilio");
            });
            if (!cleanUrls.length) {
                return undefined;
            }
            return {
                ...server,
                urls: Array.isArray(urls) ? cleanUrls : cleanUrls[0],
            };
        })
        .filter(Boolean);
}
...
export class PeerToPeer extends EventTarget {
    ...
    async handleNotification(id, content) {
        ...
        intranetRtcLog("debug", "Received RTC signaling notification", {
            peerId: id,
            event,
            channelId,
        });
        ...
        switch (event) {
            ...
            case INTERNAL_EVENT.ICE_CANDIDATE: {
                ...
                intranetRtcLog("debug", "Adding remote ICE candidate", {
                    peerId: id,
                    candidate: summarizeCandidateForLog(rtcIceCandidate),
                });
                try {
            ...
            case INTERNAL_EVENT.OFFER: {
                ...
                intranetRtcLog("debug", "Sending RTC answer", { peerId: id });
    ...
    _createPeer(id, options = {}) {
        ...
        intranetRtcLog("info", "RTCPeerConnection created", {
            peerId: id,
            selfId: this.selfId,
            channelId: this.channelId,
            iceServers: summarizeIceServersForLog(this._iceServers),
        ...
        dataChannel.addEventListener("open", () =>
            intranetRtcLog("info", "RTC data channel opened", { peerId: id })
        );
        dataChannel.addEventListener("close", () =>
            intranetRtcLog("warn", "RTC data channel closed", { peerId: id })
        );
        dataChannel.addEventListener("error", (event) =>
            intranetRtcLog("error", "RTC data channel error", {
                peerId: id,
        ...
        peerConnection.addEventListener("icecandidate", async (event) => {
            if (!event.candidate) {
                intranetRtcLog("info", "ICE candidate gathering complete", { peerId: id });
                return;
            }
            intranetRtcLog("debug", "Local ICE candidate gathered", {
                peerId: id,
                candidate: summarizeCandidateForLog(event.candidate),
            });
            if (!this.isActive || !this.peers.has(id)) {
                return;
            }
        ...
        peerConnection.addEventListener("iceconnectionstatechange", async () => {
            intranetRtcLog("info", "ICE connection state changed", {
                peerId: id,
                iceConnectionState: peerConnection.iceConnectionState,
            });
        ...
        peerConnection.addEventListener("icegatheringstatechange", () => {
            intranetRtcLog("info", "ICE gathering state changed", {
                peerId: id,
                iceGatheringState: peerConnection.iceGatheringState,
            });
            this._emitLog(
        ...
        peerConnection.addEventListener("connectionstatechange", async () => {
            intranetRtcLog("info", "Peer connection state changed", {
                peerId: id,
                connectionState: peerConnection.connectionState,
                iceConnectionState: peerConnection.iceConnectionState,
                iceGatheringState: peerConnection.iceGatheringState,
        ...
        peerConnection.addEventListener("icecandidateerror", async (error) => {
            intranetRtcLog("error", "ICE candidate error", {
                peerId: id,
        ...
        peerConnection.addEventListener("negotiationneeded", async () => {
            intranetRtcLog("debug", "Negotiation needed", {
                peerId: id,
                signalingState: peerConnection.signalingState,
            });
            ...
            intranetRtcLog("debug", "Sending RTC offer", { peerId: id, sequence: peer.sequence });
        ...
        peerConnection.addEventListener("track", async ({ transceiver, track }) => {
            intranetRtcLog("info", "Remote track event", {
                peerId: id,
                kind: track?.kind,
                id: track?.id,
