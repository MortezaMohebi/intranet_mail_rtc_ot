/** @odoo-module alias=@mail/discuss/call/common/blur_manager default=false */
...
export class BlurManager {
    ...
    constructor(
        ...
        if (!window.SelfieSegmentation) {
            this.rejectStreamPromise(new Error("Local background blur library is not loaded."));
            return;
        }
        try {
            this.selfieSegmentation = new window.SelfieSegmentation({
                locateFile: (file) => {
                    return `/intranet_mail_rtc_ot/static/lib/selfie_segmentation/${file}`;
                },
            });
        } catch (error) {
